...
try:
    while True:
        ...
        if(mode == "video") and (proximity == True):
            ...
            pool.starmap(cv2.imwrite, zip(f"Camera/images/{timeStamp}/{tag}_{img_num}_C.tif", color_image))
            pool.starmap(cv2.imwrite, zip(f"Camera/images/{timeStamp}/{tag}_{img_num}_C.tif", depth_image))
            #pool.starmap(rs.save_single_frameset,
